-- Migration: Update RPC functions to check exercises per session
-- Description: Updates create_full_program, add_session_to_program, and update_full_session
--              to check max_exercises_per_session instead of total exercises count.
--
-- ROLLBACK INSTRUCTIONS:
-- To rollback this migration:
-- 1. Restore functions from migration 20260124150000_add_limit_checks_to_rpc.sql

-- Step 1: Update create_full_program to check exercises per session
...
BEGIN
    ...
    -- Get max_exercises_per_session limit
    v_max_exercises_per_session := (v_limits->'limits'->>'max_exercises_per_session')::int;

    -- Check exercises limit per session (not total)
    ...
    LOOP
        ...
        -- Check if this session exceeds the exercises per session limit
...
-- Step 2: Update add_session_to_program to check exercises per session
...
BEGIN
    ...
    -- Check exercises limit per session if exercises are being added
    IF session_data ? 'exercises' AND jsonb_typeof(session_data->'exercises') = 'array' THEN
        ...
        -- Check if number of exercises exceeds limit per session
...
-- Step 3: Update update_full_session to check exercises per session
...
BEGIN
    ...
    -- Check exercises limit per session if exercises are being updated
...
UPDATED: Now checks max_exercises_per_session for each session, not total exercises.';
...
UPDATED: Now checks max_exercises_per_session for the new session, not total exercises.';
...
UPDATED: Now checks max_exercises_per_session for the updated session, not total exercises.';
