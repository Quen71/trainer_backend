                ...
                SELECT jsonb_agg(
                    ...
                    CASE
                        WHEN s.type = 'AMRAP' THEN
                            jsonb_build_object('duration', s.parameters->'duration')
                        WHEN s.type IN ('EMOM', 'HIIT') THEN
                            jsonb_build_object('round_number', s.parameters->'round_number') -- <<< CORRECTION ICI
                        ELSE
                            '{}'::jsonb
                    END
                )
...
BEGIN
    ...
    RETURN (
        SELECT jsonb_build_object(
            ...
            'sessions', (
                SELECT jsonb_agg(
                    ...
                    ||
                    CASE
                        -- Note: Using the exact syntax from user's working RPCs
                        WHEN s.type = 'AMRAP' THEN
                            jsonb_build_object('duration', s.parameters->'duration')
                        WHEN s.type IN ('EMOM', 'HIIT') THEN
                            jsonb_build_object('round_number', s.parameters->'round_number')
        ...
        FROM programs p
        WHERE p.id = program_id_to_update
    );
END;
$function$
;
