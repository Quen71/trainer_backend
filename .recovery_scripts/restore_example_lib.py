#!/usr/bin/env python3
"""
Restaurer les fichiers example/lib/ sélectionnés depuis le cache Cursor
"""
import json
...
files_to_restore = [
    'session_log_detail.screen.dart',
    'session_logger.screen.dart',
    'sessions_logs.screen.dart'
]
...
print("=" * 100)
print("🔄 RESTAURATION DES FICHIERS EXAMPLE/LIB MODIFIÉS")
print("=" * 100)
print()
...
for rel_path in files_to_restore:
    ...
    try:
        ...
        print(f"✅ example/lib/{rel_path}")
        print(f"   📅 Date: {file_info['date']}")
        print(f"   📏 {len(content)} caractères ({len(content) // 1024}KB) restaurés")
        print()
        restored_count += 1
