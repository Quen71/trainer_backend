#!/usr/bin/env python3
"""
Comparer les fichiers récupérés avec les versions actuelles
et présenter les différences fichier par fichier
"""
import json
...
print("=" * 100)
print("🔍 COMPARAISON DES FICHIERS RÉCUPÉRÉS AVEC LES VERSIONS ACTUELLES")
print("=" * 100)
print()
...
for category, paths in sorted(categories.items()):
    ...
    for rel_path in paths:
        ...
        print(f"\n[{count}/{total_files}] 📄 lib/models/{rel_path}")
        print("-" * 100)
        ...
        if not full_path.exists():
            ...
            if current_content.strip() == recovered_content.strip():
                ...
                if len(diff) > 0:
                    ...
                    if len(diff) > 40:
                        print(f"  ... (+{len(diff) - 40} lignes supplémentaires dans le diff)")
...
restore_file = Path('/Users/quentinlebreton/git/trainer_backend/restore_plan.json')
