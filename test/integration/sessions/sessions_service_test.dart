...
void main() {
  ...
  group('updateFullSession — Classic', () {
    test('should return a valid SessionApiResponse', () async {
      ...
      final SessionApiResponse response =
          await SessionsService.updateFullSession(session);
    ...
    test('should update session name', () async {
      ...
      final SessionApiResponse response =
          await SessionsService.updateFullSession(
    ...
    test(
      'should update exercise parameters (weight) within the session',
      () async {
        final Program created = await ProgramsService.createFullProgram(
          TestPrograms.createSimpleProgram(
            name: 'Program for Classic Exercise Update',
            exerciseCount: 2,
          ),
        );
        addTearDown(() async => ProgramsService.deleteProgram(created.id));
        ...
        final List<ClassicExercise> updatedExercises = session.exercises.map((
          ClassicExercise ex,
        ) {
          final List<ClassicExerciseSet> sets = ex.templateParameters.sets
              ...
              .toList();
          return ex.copyWith(
            templateParameters: ex.templateParameters.copyWith(sets: sets),
          );
        }).toList();
        ...
        expect(
          returned.exercises.every(
            (ClassicExercise e) => e.templateParameters.sets.every(
              (ClassicExerciseSet s) => s.weight == 999.0,
            ),
          ),
          isTrue,
        );
          ...
          await SessionsService.updateFullSession(
            session.copyWith(
              exercises: <ClassicExercise>[
                original.copyWith(name: 'Renamed Exercise'),
              ],
            ),
          );
    ...
    test(
      ...
      () async {
        ...
        final List<AmrapExercise> updatedExercises = session.exercises
            .map(
              (AmrapExercise ex) => ex.copyWith(
                templateParameters: ex.templateParameters.copyWith(
                  weight: 999.0,
                ),
              ),
            )
        ...
        expect(
          returned.exercises.every(
            (AmrapExercise e) => e.templateParameters.weight == 999.0,
          ),
          isTrue,
        );
      },
          ...
          await SessionsService.updateFullSession(
            session.copyWith(
              exercises: <AmrapExercise>[
                original.copyWith(name: 'Renamed AMRAP Exercise'),
              ],
            ),
          );
  ...
  group('updateFullSession — EMOM', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          returned.exercises.every(
            (EmomExercise e) => e.templateParameters.weight == 999.0,
          ),
          isTrue,
        );
      },
          ...
          await SessionsService.updateFullSession(
            session.copyWith(
              exercises: <EmomExercise>[
                original.copyWith(name: 'Renamed EMOM Exercise'),
              ],
            ),
          );
  ...
  group('updateFullSession — HIIT', () {
    ...
    test(
      'should update exercise parameters (effort duration) within the session',
      () async {
        final Program base = await ProgramsService.createFullProgram(
          Program.forCreation(
        ...
        addTearDown(() async => ProgramsService.deleteProgram(base.id));
        final HiitSession session = base.sessions
        ...
        final List<HiitExercise> updatedExercises = session.exercises
            .map(
              (HiitExercise ex) => ex.copyWith(
                templateParameters: ex.templateParameters.copyWith(
                  effortDuration: updatedEffort,
                ),
              ),
            )
            .toList();
        ...
        expect(
          returned.exercises.every(
            (HiitExercise e) =>
                e.templateParameters.effortDuration == updatedEffort,
          ),
          isTrue,
        );
      },
          ...
          await SessionsService.updateFullSession(
            session.copyWith(
              exercises: <HiitExercise>[
                original.copyWith(name: 'Renamed HIIT Exercise'),
              ],
            ),
          );
